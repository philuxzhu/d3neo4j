...
function appendGraph(container){
	svg = container.append("svg")
					...
					.style("touch-action", "none")  // 让浏览器不处理手势
...
function appendOutlineToNode(node){
	return node.append("circle")
				.attr("class", "outline")
				.attr("r", function(d){
					radiusMultiplier = getNodeRadiusMultiplier(d.properties.mentions || 1);
					return options.nodeRadius * radiusMultiplier;
				})
...
function appendRingToNode(node){
	return node.append("circle")
				...
				.attr("r", function(d){
					// 比outline大一些
					radiusMultiplier = getNodeRadiusMultiplier(d.properties.mentions || 1);
					return options.nodeRadius * radiusMultiplier * 1.16;
				})
				.append("title").text(function(d){
					return toString(d);
				});
...
function getNodeRadiusMultiplier(mentions) {
	if (mentions < 1)
		mentions = 1;
	if (mentions > 10)
		mentions = 10;
    const norm = (mentions - 1) / 10;
    // 限制到1~2之间，也就是1+norm
    return 1 + norm;
}
...
function dragStarted(event, d){
	// 拖动节点时页面跳动，是因为移动端触摸事件没有阻止导致页面滚动/缩放。
